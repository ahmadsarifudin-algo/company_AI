"""
AuditAgent — Internal audit, control testing, compliance gap analysis.
"""
from __future__ import annotations
import json, uuid, structlog
from app.agents.base_agent import BaseAgent
from app.agents.output_schemas import AuditOutput
from app.agents.state import AgentState
from app.core.telemetry import now_ms
logger = structlog.get_logger()

class AuditAgent(BaseAgent):
    """Performs internal audit, control testing, and compliance gap analysis."""
    name = "audit_agent"
    department = "finance"
    role = "agent"
    ...
    def _default_system_prompt(self) -> str:
        return (
            ...
            "Responsibilities:\n"
            "1) Conduct internal audit procedures\n"
            "2) Test internal controls effectiveness\n"
            "3) Identify compliance gaps\n"
            "4) Rate risk levels for findings\n\n"
            "Return ONLY valid JSON (no markdown), with keys:\n"
            '{"findings": [], "control_tests": [], "compliance_gaps": [], "risk_rating": "low"}\n'
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        result = self._validate_output(result, AuditOutput)
        artifact_id = f"audit_{task_id}_{span_id}"
        artifact = {"artifact_id": artifact_id, "type": "finance.audit.report",
            "created_at_ms": now_ms(), "trace_id": trace_id, "span_id": span_id,
            "agent": self.name, "payload": result}
        artifacts = dict(state.get("artifacts", {})); artifacts["audit_report"] = artifact
        ...
        return {**state, "status": "audit_complete", "current_agent": self.name,
