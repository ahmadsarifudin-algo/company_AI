"""DealIntelligenceAgent — Competitive analysis, battle cards, deal strategy."""
from __future__ import annotations
import json, uuid, structlog
from app.agents.base_agent import BaseAgent
from app.agents.output_schemas import DealIntelligenceOutput
from app.agents.state import AgentState
from app.core.telemetry import now_ms
logger = structlog.get_logger()

class DealIntelligenceAgent(BaseAgent):
    """Produces competitive analysis, battle cards, and deal strategies."""
    name = "deal_intelligence_agent"; department = "sales"; role = "agent"

    def _default_system_prompt(self) -> str:
        return (
            "You are a Deal Intelligence Agent in the Sales Department.\n\n"
            "Responsibilities:\n1) Analyze competitive landscape\n"
            "2) Create battle cards for sales reps\n3) Recommend deal strategies\n\n"
            "Return ONLY valid JSON:\n"
            '{"competitive_analysis": {}, "battle_card": {}, "deal_strategy": {}}\n'
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        result = self._validate_output(result, DealIntelligenceOutput)
        artifact_id = f"deal_{task_id}_{span_id}"
        artifact = {"artifact_id": artifact_id, "type": "sales.deal_intelligence.analysis",
            "created_at_ms": now_ms(), "trace_id": trace_id, "span_id": span_id,
            "agent": self.name, "payload": result}
        artifacts = dict(state.get("artifacts", {})); artifacts["deal_intelligence"] = artifact
        ...
        backend_output = self._build_output(
            ...
            summary=result.get("summary", "Deal intelligence completed."),
        ...
        return {**state, "status": "deal_intelligence_complete", "current_agent": self.name,
