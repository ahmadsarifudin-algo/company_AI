"""SalesForecastingAgent — Pipeline forecasting, quota tracking, pipeline health."""
from __future__ import annotations
import json, uuid, structlog
from app.agents.base_agent import BaseAgent
from app.agents.output_schemas import SalesForecastingOutput
from app.agents.state import AgentState
from app.core.telemetry import now_ms
logger = structlog.get_logger()

class SalesForecastingAgent(BaseAgent):
    """Forecasts pipeline, tracks quotas, and measures pipeline health."""
    name = "sales_forecasting_agent"; department = "sales"; role = "agent"

    def _default_system_prompt(self) -> str:
        return (
            "You are a Sales Forecasting Agent in the Sales Department.\n\n"
            "Responsibilities:\n1) Forecast sales pipeline\n"
            "2) Track quota attainment\n3) Measure pipeline health metrics\n\n"
            "Return ONLY valid JSON:\n"
            '{"forecast": {}, "quota_tracking": [], "pipeline_health": {}}\n'
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        result = self._validate_output(result, SalesForecastingOutput)
        artifact_id = f"sales_fcst_{task_id}_{span_id}"
        artifact = {"artifact_id": artifact_id, "type": "sales.forecasting.projection",
            "created_at_ms": now_ms(), "trace_id": trace_id, "span_id": span_id,
            "agent": self.name, "payload": result}
        artifacts = dict(state.get("artifacts", {})); artifacts["sales_forecast"] = artifact
        ...
        backend_output = self._build_output(
            ...
            summary=result.get("summary", "Sales forecast completed."),
        ...
        return {**state, "status": "sales_forecasting_complete", "current_agent": self.name,
