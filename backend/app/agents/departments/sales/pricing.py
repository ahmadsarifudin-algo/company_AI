"""PricingAgent — Pricing optimization, discount approval flags."""
from __future__ import annotations
import json, uuid, structlog
from app.agents.base_agent import BaseAgent
from app.agents.output_schemas import PricingOutput
from app.agents.state import AgentState
from app.core.telemetry import now_ms
logger = structlog.get_logger()

class PricingAgent(BaseAgent):
    """Optimizes pricing strategies and flags discount approvals."""
    name = "pricing_agent"; department = "sales"; role = "agent"

    def _default_system_prompt(self) -> str:
        return (
            "You are a Pricing Agent in the Sales Department.\n\n"
            "Responsibilities:\n1) Optimize pricing strategies\n"
            "2) Analyze discount requests\n3) Flag if approval is required\n\n"
            "Return ONLY valid JSON:\n"
            '{"pricing": {}, "approval_required": false}\n'
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        response = await self.call_llm([{"role": "system", "content": self.get_system_prompt()},
            {"role": "user", "content": state.get("task_description", "")}], temperature=0.2, ctx=ctx)
        content = "" if response is None else getattr(response, "content", "") or ""
        result = self._safe_json_loads(content)
        result = self._validate_output(result, PricingOutput)
        artifact_id = f"pricing_{task_id}_{span_id}"
        artifact = {"artifact_id": artifact_id, "type": "sales.pricing.analysis",
            "created_at_ms": now_ms(), "trace_id": trace_id, "span_id": span_id,
            "agent": self.name, "payload": result}
        artifacts = dict(state.get("artifacts", {})); artifacts["pricing_analysis"] = artifact
        ...
        backend_output = self._build_output(
            ...
            summary=result.get("summary", "Pricing analysis completed."),
        ...
        return {**state, "status": "pricing_complete", "current_agent": self.name,
