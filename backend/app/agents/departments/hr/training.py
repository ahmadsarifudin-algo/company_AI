"""TrainingAgent — Skill gap analysis, learning paths, training ROI."""
from __future__ import annotations
import json, uuid, structlog
from app.agents.base_agent import BaseAgent
from app.agents.output_schemas import TrainingOutput
from app.agents.state import AgentState
from app.core.telemetry import now_ms
logger = structlog.get_logger()

class TrainingAgent(BaseAgent):
    """Analyzes skill gaps, designs learning paths, measures training ROI."""
    name = "training_agent"; department = "hr"; role = "agent"

    def _default_system_prompt(self) -> str:
        return (
            "You are a Training Agent in the HR Department.\n\n"
            "Responsibilities:\n1) Identify skill gaps across teams\n"
            "2) Design personalized learning paths\n3) Calculate training ROI\n\n"
            "Return ONLY valid JSON:\n"
            '{"skill_gaps": [], "learning_path": [], "training_roi": {}}\n'
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        result = self._validate_output(result, TrainingOutput)
        artifact_id = f"train_{task_id}_{span_id}"
        artifact = {"artifact_id": artifact_id, "type": "hr.training.plan",
            "created_at_ms": now_ms(), "trace_id": trace_id, "span_id": span_id,
            "agent": self.name, "payload": result}
        artifacts = dict(state.get("artifacts", {})); artifacts["training"] = artifact
        ...
        backend_output = self._build_output(
            ...
            summary=result.get("summary", "Training plan completed."),
        ...
        return {**state, "status": "training_complete", "current_agent": self.name,
