"""PerformanceAgent — KPI tracking, performance reviews, talent grid."""
from __future__ import annotations
import json, uuid, structlog
from app.agents.base_agent import BaseAgent
from app.agents.output_schemas import PerformanceOutput
from app.agents.state import AgentState
from app.core.telemetry import now_ms
logger = structlog.get_logger()

class PerformanceAgent(BaseAgent):
    """Tracks KPIs, generates performance reviews, builds talent grids."""
    name = "performance_agent"; department = "hr"; role = "agent"

    def _default_system_prompt(self) -> str:
        return (
            "You are a Performance Agent in the HR Department.\n\n"
            "Responsibilities:\n1) Track and analyze KPIs\n"
            "2) Generate performance review summaries\n"
            "3) Build 9-box talent grids\n4) Recommend development plans\n\n"
            "Return ONLY valid JSON:\n"
            '{"kpi_summary": {}, "performance_reviews": [], "talent_grid": {}, "recommendations": []}\n'
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        result = self._validate_output(result, PerformanceOutput)
        artifact_id = f"perf_{task_id}_{span_id}"
        artifact = {"artifact_id": artifact_id, "type": "hr.performance.review",
            "created_at_ms": now_ms(), "trace_id": trace_id, "span_id": span_id,
            "agent": self.name, "payload": result}
        artifacts = dict(state.get("artifacts", {})); artifacts["performance"] = artifact
        ...
        backend_output = self._build_output(
            ...
            summary=result.get("summary", "Performance analysis completed."),
        ...
        return {**state, "status": "performance_complete", "current_agent": self.name,
