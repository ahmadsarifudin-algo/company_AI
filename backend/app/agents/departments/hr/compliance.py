"""ComplianceAgent — Labor law compliance, risk assessment, action items."""
from __future__ import annotations
import json, uuid, structlog
from app.agents.base_agent import BaseAgent
from app.agents.output_schemas import ComplianceOutput
from app.agents.state import AgentState
from app.core.telemetry import now_ms
logger = structlog.get_logger()

class ComplianceAgent(BaseAgent):
    """Monitors labor law compliance, assesses risk, issues action items."""
    name = "compliance_agent"; department = "hr"; role = "agent"

    def _default_system_prompt(self) -> str:
        return (
            "You are a Compliance Agent in the HR Department.\n\n"
            "Responsibilities:\n1) Check labor law compliance\n"
            "2) Assess compliance risk levels\n3) Issue corrective action items\n\n"
            "Return ONLY valid JSON:\n"
            '{"compliance_check": {}, "risk_level": "low", "action_items": []}\n'
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        result = self._validate_output(result, ComplianceOutput)
        artifact_id = f"comply_{task_id}_{span_id}"
        artifact = {"artifact_id": artifact_id, "type": "hr.compliance.check",
            "created_at_ms": now_ms(), "trace_id": trace_id, "span_id": span_id,
            "agent": self.name, "payload": result}
        artifacts = dict(state.get("artifacts", {})); artifacts["compliance"] = artifact
        ...
        return {**state, "status": "compliance_complete", "current_agent": self.name,
