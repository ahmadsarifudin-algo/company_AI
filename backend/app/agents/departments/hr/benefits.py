"""BenefitsAgent — Eligibility analysis, enrollment status, cost summaries."""
from __future__ import annotations
import json, uuid, structlog
from app.agents.base_agent import BaseAgent
from app.agents.output_schemas import BenefitsOutput
from app.agents.state import AgentState
from app.core.telemetry import now_ms
logger = structlog.get_logger()

class BenefitsAgent(BaseAgent):
    """Analyzes benefits eligibility, enrollment, and cost impact."""
    name = "benefits_agent"; department = "hr"; role = "agent"

    def _default_system_prompt(self) -> str:
        return (
            "You are a Benefits Agent in the HR Department.\n\n"
            "Responsibilities:\n1) Determine benefits eligibility\n"
            "2) Track enrollment status\n3) Summarize cost impact\n\n"
            "Return ONLY valid JSON:\n"
            '{"eligibility": [], "enrollment_status": {}, "cost_summary": {}}\n'
        )
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        result = self._validate_output(result, BenefitsOutput)
        artifact_id = f"benefits_{task_id}_{span_id}"
        artifact = {"artifact_id": artifact_id, "type": "hr.benefits.analysis",
            "created_at_ms": now_ms(), "trace_id": trace_id, "span_id": span_id,
            "agent": self.name, "payload": result}
        artifacts = dict(state.get("artifacts", {})); artifacts["benefits"] = artifact
        ...
        backend_output = self._build_output(
            ...
            summary=result.get("summary", "Benefits analysis completed."),
        ...
        return {**state, "status": "benefits_complete", "current_agent": self.name,
