...
from app.core.http_gateway import http_post
...
async def send_telegram_handler(
    ...
    try:
        if photo_url:
            ...
            result = await http_post(f"{base_url}/sendPhoto", json=payload, timeout=15)
        else:
            # Send text message
            payload = {
                "chat_id": chat_id,
            ...
            if parse_mode:
                payload["parse_mode"] = parse_mode
            result = await http_post(f"{base_url}/sendMessage", json=payload, timeout=15)
