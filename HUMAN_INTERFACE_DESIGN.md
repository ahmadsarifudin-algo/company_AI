...
- **Workflow**:
  1.  Agent reaches a **Checkpoint** (e.g., `deploy_to_prod`, `transfer_funds`).
  2.  Execution **Pauses** automatically.
  3.  Dashboard shows a "Blocked" task with context (Plan, Proposed Action, Risk).
  4.  Human operator clicks **Approve** or **Reject** (with feedback).
  5.  Execution **Resumes** with the human's input.
- **Technology**: LangGraph Checkpoints (`interrupt_before=["tool_call"]`) + FastAPI `/approve/{thread_id}`.
...
- **Role-Based Access**: Only "Admins" can see the Approval Queue.
- **Audit Logging**: Every "Approve" click logs the user ID, timestamp, and decision to the immutable ledger.
